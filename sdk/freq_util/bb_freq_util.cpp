...
void k_qsort(float l[],int low,int high)
{
	int prvotloc;
	if(low<high)
	{
        prvotloc=partions(l,low,high);    //将第一次排序的结果作为枢轴
        k_qsort(l,low,prvotloc-1); //递归调用排序 由low 到prvotloc-1
        k_qsort(l,prvotloc+1,high); //递归调用排序 由 prvotloc+1到 high
...
{
	k_qsort(l,1,n); //第一个作为枢轴 ，从第一个排到第n个
}
